...
const layout = Utils.derive([bar.position, wallpaper.position], (bar, qs) =>
    `${bar}-${qs}` as const,
)
...
function Wallpaper() {
    ...
    function getAllFiles(dirPath: string, arrayOfFiles: string[] = []): string[] {
        const fileEnum = Gio.File.new_for_path(dirPath).enumerate_children('standard::*', Gio.FileQueryInfoFlags.NONE, null);
        let fileInfo;
    ...
    const wallpaperItems = wallpaperFiles.map(filePath => 
        Widget.Button({
            ...
            on_clicked: () => {
                ...
                console.log(`${GLib.get_home_dir()}/.config/hypr/scripts/wallpaper.sh -s ${filePath}`)
                sh(`${GLib.get_home_dir()}/.config/hypr/scripts/wallpaper.sh -s ${filePath}`)
            },
        })
    );

    const sortedWallpaperItems = wallpaperItems.reduce((resultArray: Widget.Button[][], item: Widget.Button, index: number) => { 
        ...
        return resultArray
    }, [] as Widget.Button[][]).map(item => Widget.Box({
        vertical: true,
        children: item}));
...
const WallpaperWindow = () => PopupWindow({
    name: "wallpaper",
